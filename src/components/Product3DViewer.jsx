...
import { OrbitControls, PerspectiveCamera, Environment, ContactShadows, Stage } from '@react-three/drei'
import { X, RotateCw, Maximize2, Minimize2, Download } from 'lucide-react'
...
const ProductModel = ({ productId, productName, color = '#3b82f6', rotation, useGLTF = true }) => {
  const [modelError, setModelError] = useState(false)
  const modelConfig = getModelConfig(productId)

  // Try to load GLTF model first
  if (useGLTF && !modelError) {
    return (
  ...
  const getProceduralModel = () => {
    switch (productId) {
      ...
      case 5: return <DesignerBackpack color={color} rotation={rotation} />
      case 6: return <CrossbodyBag color={color} rotation={rotation} />
      case 7: return <ClassicTShirt color={color} rotation={rotation} />
      case 8: return <DenimJacket color={color} rotation={rotation} />
      case 9: return <SummerDress color={color} rotation={rotation} />
      case 10: return <SmartWatchPro color={color} rotation={rotation} />
      case 11: return <ClassicAnalogWatch color={color} rotation={rotation} />
      case 12: return <SportWatch color={color} rotation={rotation} />
